...
const SignUp = () => {
    ...
    const uploadFields = () => {
        ...
        fetch("http://localhost:5000/signup", {
            ...
            body: JSON.stringify({
                firstname, lastname, password, email, city, stateName, mobile, age, pic: url, isAdmin: userRole === "Admin"
            })
        }).then(res => res.json())
            .then(data => {
                if (data.error) {
    ...
    return (
        <div className='col-5' style={{ margin: 'auto' }}>
            <div className="card px-5 py-2" style={{ margin: "5%" }}>
                ...
                <div className="row">
                    <div className="col">
                        <select className="form-select" style={{ marginTop: '5px', width: '100%' }} value={userRole} onChange={(e) => setUserRole(e.target.value)}>
                            {userType.map((type) => (
